...
def operation_3b(df: DataFrame): DataFrame = {
        ...
        df_with_diff.withColumn("modelYearDemandClass",when($"diff" > 0, when($"diff" > 50, "very_high").otherwise("high")).otherwise(when($"diff" < -50, "very_low").otherwise("low"))).select(col("firstSeen"),col("lastS$}
...
fs.rename(new Path("hdfs://master:9000/geo/tmp3.csv/" + file), new Path("hdfs://master:9000/geo/tmp_3.csv"))
fs.delete(new Path("hdfs://master:9000/geo/tmp3/"), true)
